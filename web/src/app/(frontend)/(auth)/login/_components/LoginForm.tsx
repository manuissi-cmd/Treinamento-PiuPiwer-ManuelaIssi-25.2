  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await authClient.signIn.email({
        email,
        password,
        callbackURL: "/home", // <<<<<< redireciona para /home
      });
  ...
  return (
    <div className="lg:w-[90%] xl:w-[80%]">
      ...
      <form className="mt-6" onSubmit={handleSubmit}>
        ...
        {/* garante que o formulário envia */}
        <CredentialsButton type="submit" className="mt-6" disabled={loading}>
      ...
      <Link href="/cadastro" className="block w-fit mt-8 text-sm group">
        Ainda não tem uma conta? <span className="text-pink-500 colorTransition border-b border-transparent group-hover:border-pink-500">Cadastre-se</span>
      </Link>
    </div>
  );
}
